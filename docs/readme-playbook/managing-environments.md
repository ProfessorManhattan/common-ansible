...
We created a bash script that will set up all the symlinks for you. You can run this by running `bash environments/dev/link.sh`. This command will symlink all the folders in selected environment to their appropriate locations in the base project. Alternatively, if you have Task installed or already ran `bash .start.sh`, you can switch environments with an interactive prompt by running:
